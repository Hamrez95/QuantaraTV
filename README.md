...
Copy [`src/quantara_vision.pine`](src/quantara_vision.pine) into TradingView Pine Editor, save, choose **Add to chart**, and start with `Balanced / Intraday / Compact / Balanced`. Version 0.2.1 keeps the interface English-only, enables native hover help, keeps filled risk zones/session shading/pivot trendlines off initially, and shows only active trade-plan lines.
